...
section "5. ALB (Application Load Balancer) [배점: 3점]  ※ 5-1(1점) 5-2(1.5점) 5-3(0.5점)"

ALB_NAME="${PLAYER_ID}-book-alb"

# 5-1: ALB internet-facing 생성 및 ECS Service 연결 확인 (1점)
echo -e "\n${BOLD}[5-1] ALB internet-facing 생성 및 ECS Service 연결 확인 (1점)${NC}"
...
# TG_ARN 저장 (5-2, 6-2에서 사용) — 문제지: TG 이름 미지정 → ALB 연결 TG로 조회
TG_ARN=$(aws elbv2 describe-target-groups \
  --load-balancer-arn "$ALB_ARN" \
  --query "TargetGroups[0].TargetGroupArn" --output text --region $REGION 2>/dev/null || echo "None")

# 5-2: Listener, Target Group 및 Target Health 확인 (1.5점)
echo -e "\n${BOLD}[5-2] Listener HTTP:80, TG HTTP:8080(IP), Target Health 확인 (1.5점)${NC}"
...
if [ "$ALB_ARN" = "None" ] || [ -z "$ALB_ARN" ]; then
  ...
  if [ "$HAS_LISTENER" -ge 1 ] && [ "$HAS_TG" -ge 1 ] && [ "$HEALTHY_COUNT" -ge 1 ]; then
    pass "5-2" "1.5" "Listener HTTP:80, TG HTTP:8080(ip), Healthy: ${HEALTHY_COUNT}개"
  else
    fail "5-2" "Listener:80=${HAS_LISTENER}개, TG:8080(ip)=${HAS_TG}개, Healthy=${HEALTHY_COUNT}개"
  fi
fi

# 5-3: 보안그룹 규칙 확인 (0.5점)
echo -e "\n${BOLD}[5-3] ALB SG(인바운드 HTTP:80) 및 ECS SG(인바운드 TCP:8080 소스=ALB SG) 확인 (0.5점)${NC}"
...
if [ "$ALB_OK" -ge 1 ] && [ "$ECS_OK" -eq 1 ]; then
  pass "5-3" "0.5" "ALB SG: HTTP:80/0.0.0.0/0, ECS SG: TCP:8080 소스=$ALB_SG_ID"
else
...
section "7. CloudWatch Logs [배점: 4점]  ※ 7-1(1점) 7-2(1.5점) 7-3(1.5점)"
...
# 7-2: 로그 스트림 생성 및 로그 수집 확인 (1.5점)
echo -e "\n${BOLD}[7-2] 로그 스트림(ecs/ 접두어) 생성 및 로그 이벤트 수집 확인 (1.5점)${NC}"
...
for i in 1 2 3; do
  LOG_EVENT_COUNT=$(aws logs filter-log-events \
    ...
    --no-paginate \
    --query "length(events)" --output text --region $REGION 2>/dev/null | head -1 || echo "0")
  LOG_EVENT_COUNT="${LOG_EVENT_COUNT:-0}"
  [ "${LOG_EVENT_COUNT}" -ge 1 ] 2>/dev/null && break
  [ $i -lt 3 ] && info "로그 수집 대기 중... (${i}/3, 60초 후 재시도)" && sleep 60
done

if [ "${STREAM_COUNT:-0}" -ge 1 ] 2>/dev/null && [ "${LOG_EVENT_COUNT:-0}" -ge 1 ] 2>/dev/null; then
  pass "7-2" "1.5" "ecs/ 스트림 ${STREAM_COUNT}개, 로그 이벤트 ${LOG_EVENT_COUNT}건 수집 확인"
else
  fail "7-2" "ecs/ 스트림: ${STREAM_COUNT:-0}개, 로그 이벤트: ${LOG_EVENT_COUNT:-0}건 (모두 1 이상 필요)"
fi

# 7-3: awslogs 드라이버 및 로그 그룹 설정 확인 (1.5점)
echo -e "\n${BOLD}[7-3] Task Definition logConfiguration awslogs 드라이버 설정 확인 (1.5점)${NC}"

if [ "$TD" = "None" ] || [ -z "$TD" ]; then
  fail "7-3" "Task Definition 조회 불가"
else
  ...
  if [ "$HAS_AWSLOGS" -ge 1 ] && [ "$HAS_LOG_GROUP" -ge 1 ]; then
    pass "7-3" "1.5" "logDriver:awslogs, awslogs-group:${LOG_GROUP}"
  else
    fail "7-3" "logDriver 또는 awslogs-group 설정 불일치 (현재: $LOG_CONFIG)"
  fi
fi
