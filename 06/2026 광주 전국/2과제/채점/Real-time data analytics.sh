...
echo =====2-3=====
OFFSET=$(/opt/kafka/bin/kafka-get-offsets.sh --bootstrap-server localhost:9092 \
  --topic error-stats --time -1 | grep ":0:" | awk -F: '{print $3-1}')
/opt/kafka/bin/kafka-console-consumer.sh --topic error-stats --partition 0 \
  --bootstrap-server localhost:9092 --max-messages 1 --offset $OFFSET 2>/dev/null
echo

echo =====2-4=====
OFFSET=$(/opt/kafka/bin/kafka-get-offsets.sh --bootstrap-server localhost:9092 \
  --topic high-latency --time -1 | grep ":0:" | awk -F: '{print $3-1}')
/opt/kafka/bin/kafka-console-consumer.sh --topic high-latency --partition 0 \
  --bootstrap-server localhost:9092 --max-messages 1 --offset $OFFSET 2>/dev/null
echo
