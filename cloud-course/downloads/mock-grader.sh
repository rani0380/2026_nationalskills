...
ENDPOINT="${ENDPOINT%/}"; RID="$(printf '%012d' "$(date +%s)")"; UUID="$(cat /proc/sys/kernel/random/uuid)"
...
run_load(){ local k="$1" lim="$2" ok=0 fast=0 i code sec path method body; for ((i=1;i<=REQUESTS;i++)); do case "$k" in user) method=GET; path="/v1/user?email=${EMAIL}&requestid=${RID}&uuid=${UUID}"; body="";; product) method=GET; path="/v1/product?id=${PID}&requestid=${RID}&uuid=${UUID}"; body="";; stress) method=POST; path="/v1/stress?requestid=${RID}&uuid=${UUID}"; body="$SB";; esac; read -r code sec <<<"$(request "$method" "$path" "$body")"; [[ "$code" =~ ^2 ]] && ((ok+=1)); if [[ "$code" =~ ^2 ]] && awk -v t="$sec" -v l="$lim" 'BEGIN{exit !(t<=l)}'; then ((fast+=1)); fi; done; awk -v n="$REQUESTS" -v s="$ok" -v f="$fast" 'BEGIN{printf "%.2f %.2f",s*100/n,f*100/n}'; }
...
EX=0; for ((i=1;i<=REQUESTS;i++)); do read -r C1 _ <<<"$(request GET '/v1/user?email=invalid')"; read -r C2 _ <<<"$(request GET "/v1/none?requestid=${RID}&uuid=${UUID}")"; [[ "$C1" == 403 ]] && ((EX+=1)); [[ "$C2" == 404 ]] && ((EX+=1)); done
